#!/usr/bin/env python3
"""
测试脚本：验证 knowledge-subscription 内容样例包质量 v6
任务ID: a03bf6d8
运行: pytest tests/test_sample_pack_a03bf6d8.py -v
"""
...
class TestFileExistence:
    ...
    def test_free_preview_exists(self):
        assert (SAMPLE_DIR / "free_preview_v6.md").exists()

    def test_premium_catalog_exists(self):
        assert (SAMPLE_DIR / "premium_catalog_v5.md").exists()

    def test_week1_samples_exist(self):
        days = ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"]
        for day in days:
            assert (WEEK_DIR / f"{day}_v6.md").exists()

    def test_data_json_exists(self):
        assert (SAMPLE_DIR / "data_v6.json").exists()

    def test_delivery_checklist_exists(self):
        assert (DOCS_DIR / "delivery_checklist.md").exists()

    def test_generator_exists(self):
        assert (BASE_DIR / "app" / "sample_pack_generator_a03bf6d8.py").exists()

    def test_radar_script_exists(self):
        assert (WEEK_DIR / "resources" / "scripts" / "opportunity_radar.py").exists()
...
class TestContentQuality:
    ...
    def free_preview(self):
        return (SAMPLE_DIR / "free_preview_v6.md").read_text(encoding="utf-8")

    @pytest.fixture(scope="class")
    def premium_catalog(self):
        return (SAMPLE_DIR / "premium_catalog_v5.md").read_text(encoding="utf-8")

    @pytest.fixture(scope="class")
    def data_json(self):
        with open(SAMPLE_DIR / "data_v6.json", "r", encoding="utf-8") as f:
            return json.load(f)
    ...
    def test_week_reports_have_risk_disclaimer(self):
        for day in ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"]:
            text = (WEEK_DIR / f"{day}_v6.md").read_text(encoding="utf-8")
            assert "风险" in text
...
class TestRunnableCode:
    """验证附赠代码可运行"""

    def test_opportunity_radar_script_runs(self):
        script_path = WEEK_DIR / "resources" / "scripts" / "opportunity_radar.py"
    ...
    def test_generator_script_runs(self):
        script_path = BASE_DIR / "app" / "sample_pack_generator_a03bf6d8.py"
        assert script_path.exists()
        import subprocess
        result = subprocess.run(
            ["python", "-m", "py_compile", str(script_path)],
