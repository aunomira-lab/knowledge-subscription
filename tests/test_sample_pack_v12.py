#!/usr/bin/env python3
"""
Test suite for knowledge-subscription sample pack v12
Task: 889b251b
"""
...
def test_free_preview_exists():
    fp = SAMPLE_DIR / "free_preview_v12.md"
    assert fp.exists(), "free_preview_v12.md missing"
    text = fp.read_text(encoding="utf-8")
    assert "免费试看" in text
    assert "¥99/月" in text
    assert "889b251b" in text
    assert "立即行动" in text
    assert "v12.0" in text
    print("✅ free_preview_v12.md passed")


def test_premium_catalog_exists():
    pc = SAMPLE_DIR / "premium_catalog_v12.md"
    assert pc.exists(), "premium_catalog_v12.md missing"
    ...
    assert "889b251b" in text
    assert "v12.0" in text
    print("✅ premium_catalog_v12.md passed")


def test_week1_samples_exist():
    days = ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"]
    for day in days:
        f = WEEK1_DIR / f"{day}_v12.md"
        assert f.exists(), f"{day}_v12.md missing"
        text = f.read_text(encoding="utf-8")
        assert "日报" in text or "专题" in text or "测评" in text
        assert len(text) > 1000, f"{day}_v12.md too short ({len(text)} bytes)"
    print("✅ week1_samples passed")


def test_data_json_exists():
    dj = SAMPLE_DIR / "data_v12.json"
    assert dj.exists(), "data_v12.json missing"
    data = json.loads(dj.read_text(encoding="utf-8"))
    assert "opportunities" in data
    assert len(data["opportunities"]) >= 6
    assert data["meta"]["task_id"] == "889b251b"
    assert data["meta"]["version"] == "v12.0"
    print("✅ data_v12.json passed")
...
def test_code_generator_exists():
    gen = PROJECT_DIR / "app" / "sample_pack_generator_v12.py"
    assert gen.exists(), "sample_pack_generator_v12.py missing"
    text = gen.read_text(encoding="utf-8")
    assert "889b251b" in text
    assert "def generate_all" in text
    assert "v12.0" in text
    print("✅ sample_pack_generator_v12.py passed")
...
def test_all():
    ...
    print("\n🎉 All tests passed for task 889b251b v12")
