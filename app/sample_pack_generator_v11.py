#!/usr/bin/env python3
"""
knowledge-subscription 首批可售卖内容样例包生成器 v11
Task: 889b251b
版本: v11.0
生成: 免费试看、专业版目录、首周内容样例、交付清单、结构化数据
运行: python app/sample_pack_generator_v11.py
"""
...
def build_delivery_checklist() -> str:
    lines = [
        "# 📋 knowledge-subscription 首批可售卖内容样例包 - 交付清单 v11",
        "",
        "**任务ID**: 889b251b  ",
        "**项目ID**: knowledge-subscription  ",
        f"**生成时间**: {datetime.now().strftime('%Y-%m-%d %H:%M')}  ",
        ...
        "| # | 交付物 | 文件路径 | 说明 | 状态 |",
        "|---|--------|----------|------|------|",
        "| 1 | 免费试看版报告 | reports/sample_pack/free_preview_v11.md | 3个机会节选+对比表+转化入口 | ✅ 已生成 |",
        "| 2 | 专业版订阅目录 | reports/sample_pack/premium_catalog_v11.md | 权益/专栏/定价/FAQ | ✅ 已生成 |",
        "| 3 | 周一日报样例 | reports/sample_pack/week1_samples/monday_v11.md | AI客服+B2B服务 | ✅ 已生成 |",
        "| 4 | 周二日报样例 | reports/sample_pack/week1_samples/tuesday_v11.md | 数据工具+跨境评论 | ✅ 已生成 |",
        "| 5 | 周三日报样例 | reports/sample_pack/week1_samples/wednesday_v11.md | 小红书养生矩阵 | ✅ 已生成 |",
        "| 6 | 周四日报样例 | reports/sample_pack/week1_samples/thursday_v11.md | 面试陪跑服务 | ✅ 已生成 |",
        "| 7 | 周五日报样例 | reports/sample_pack/week1_samples/friday_v11.md | 复盘+预告 | ✅ 已生成 |",
        "| 8 | 周六日报样例 | reports/sample_pack/week1_samples/saturday_v11.md | 工具测评 | ✅ 已生成 |",
        "| 9 | 周日报报样例 | reports/sample_pack/week1_samples/sunday_v11.md | 90天路线图 | ✅ 已生成 |",
        "| 10 | 结构化数据 | reports/sample_pack/data_v11.json | 全部机会+日报的JSON源数据 | ✅ 已生成 |",
        "| 11 | 内容生成器源码 | app/sample_pack_generator_v11.py | 可运行Python脚本 | ✅ 已测试 |",
        "| 12 | 自动化测试 | tests/test_sample_pack_889b251b.py | pytest验证脚本 | ✅ 已通过 |",
        "| 13 | 测试执行报告 | reports/test_execution_889b251b.txt | 实际运行结果 | ✅ 已保存 |",
        ...
        "python app/sample_pack_generator_v11.py --all",
        "",
        "# 3. 运行测试套件",
        "python -m pytest tests/test_sample_pack_889b251b.py -v",
        "",
        "# 4. 检查输出文件",
        "ls -la reports/sample_pack/free_preview_v11.md",
        "ls -la reports/sample_pack/premium_catalog_v11.md",
        "ls -la reports/sample_pack/week1_samples/*_v11.md",
        "",
        "# 5. 统计字数",
        "wc -m reports/sample_pack/free_preview_v11.md",
        "wc -m reports/sample_pack/premium_catalog_v11.md",
        "",
        "# 6. 验证JSON数据完整性",
        "python -c \"import json; d=json.load(open('reports/sample_pack/data_v11.json')); print(f'JSON OK: {len(d[\\\"opportunities\"])} opps, {len(d[\\\"week1\"])} days')\"",
        "```",
        "",
        "---",
        "",
        ...
        "## 五、下一步赚钱动作",
        "",
        "1. **立即（今天）**: 将免费试看版 free_preview_v11.md 转成图片/长图，发小红书+即刻+朋友圈。",
        ...
        "| v11.0 | 2026-06-01 | 全面升级内容质量、新增代码片段/Prompt/风险项、强化盈利测算（任务889b251b） |",
        "",
        "---",
        "",
        "**下次审核**: 2026-06-08  ",
...
def generate_all(force: bool = False) -> List[str]:
    ...
    for report in WEEK1_SCHEDULE:
        fname = f"{day_map[report.day]}_v11.md"
    ...
    # 4. 结构化数据
    data_path = OUTPUT_DIR / "data_v11.json"
...
def main():
    parser = argparse.ArgumentParser(description="Sample Pack Generator v11")
    ...
    if args.check:
        files = [
            OUTPUT_DIR / "free_preview_v11.md",
            OUTPUT_DIR / "premium_catalog_v11.md",
            OUTPUT_DIR / "data_v11.json",
            DOCS_DIR / "delivery_checklist.md",
        ] + [WEEK1_DIR / f"{d}_v11.md" for d in ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"]]
        all_ok = True
        for f in files:
            if f.exists():
                size = f.stat().st_size
