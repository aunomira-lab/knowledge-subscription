#!/usr/bin/env python3
"""
knowledge-subscription 首批可售卖内容样例包生成器 v5
任务ID: 994f3629
版本: v5.0
生成: 免费试看版、专业版目录、首周7天日报样例、交付清单、结构化数据
运行: python app/sample_pack_generator_v5.py
"""
...
OPPORTUNITIES = [
    {
        ...
        "data_sources": [
            "Dify GitHub 44k+ stars，企业版定价 $199/月，验证 B2B 需求",
            "淘宝/拼多多商家日均咨询 200-2000 条，人工客服成本 ¥3,000-8,000/月",
            "Coze 国内生态月活超 500 万，Bot 商店头部应用日调用 10 万+",
...
先输出文件结构，再逐个输出完整代码。""",
    ...
    },
    {
        ...
        "data_sources": [
            "HeyGen 月活 1000 万+，企业版 $89/月，验证市场需求",
            "抖音 '数字人带货' 话题播放量超 50 亿，头部账号月 GMV 百万+",
...
总字数控制在 200 字以内。""",
    ...
    },
    {
        ...
        "data_sources": [
            ...
            "竞品 Glasp 200 万用户、$600 万融资；Hypothesis 开源但 UI 老旧",
...
# 6. 运行 pytest 内容质量测试
pytest tests/test_sample_pack.py -v --tb=short > reports/pytest_output_{TASK_ID}.txt 2>&1
```
...
| v5.0 | 2026-05-24 | 全新 6 个机会（AI Agent / Cursor / 数字人 / 陪跑营）、Prompt 模板、更强转化设计（任务 {TASK_ID}） |
...
def main():
    ...
    print("[1/12] 生成免费试看版 v5 ...")
    fp_path = OUTPUT_DIR / "free_preview_v5.md"
    write_file(fp_path, generate_free_preview())
    files_created.append(fp_path)

    print("[2/12] 生成专业版目录 v4 ...")
    pc_path = OUTPUT_DIR / "premium_catalog_v4.md"
    ...
    for i, sched in enumerate(SCHEDULE, 3):
        ...
        print(f"[{i}/12] 生成{sched['day']}日报 ({en_day}_v5.md) ...")
        d_path = WEEK_DIR / f"{en_day}_v5.md"
        write_file(d_path, generate_daily_report(sched))
        files_created.append(d_path)

    print("[10/12] 导出结构化数据 v5 ...")
    json_path = OUTPUT_DIR / "data_v5.json"
