#!/usr/bin/env python3
"""
knowledge-subscription 首批可售卖内容样例包生成器 v8
任务ID: d03eb155
版本: v8.0
生成: 免费试看版、专业版目录、首周7天日报样例、交付清单、结构化数据
运行: python app/sample_pack_generator_d03eb155.py
"""
...
OPPORTUNITIES = [
    {
        ...
        "margin\": \">85%",
        "summary\": \"使用 Vapi、Retell 或 Bland 搭建 AI 语音电话 Agent，为房产中介、教培机构、医美诊所提供自动预约、回访、邀约服务。替代人工外呼团队，按通话分钟数或坐席数收费。",
        "data_sources": [
            "Vapi 2026年5月公开数据：日处理通话超 2,000 万分钟，企业客户数 12,000+",
            "Retell AI 完成 B 轮 $42M 融资，ARR 年增 600%，验证语音 Agent 赛道爆发",
            "国内房产中介单店月均外呼 5,000-20,000 通，人工成本 ¥6,000-15,000/人",
            "1688 上 'AI 外呼系统' 月销 500+ 套，均价 ¥499，但产品体验差、复购率低",
        ],
        "action_steps": [
            "注册 Vapi 或 Retell 免费账户，熟悉语音工作流：接听/外呼 -> STT -> LLM 决策 -> TTS 回复 -> 挂断/转人工",
            ...
            "在电销行业社群（房产/教培/医美）发试用帖：'7 天免费试用，不替换现有系统，只补夜间/高峰期漏接'，转化率目标 10%，定价 ¥0.15/分钟 或 ¥999/坐席/月",
        ],
        "prompt_template": """你是一位专业的【行业】电话销售顾问。请根据以下话术知识库，用自然、亲切的口语化中文与客户通话。语速适中，必要时使用停顿（用"..."表示）。
...
请生成下一句回复（仅输出要说的内容，不要加任何标签或解释）。如果判断需要转人工，请只说"转人工"。""",
    ...
    },
    {
        ...
        "margin\": \">90%",
        "summary\": \"利用 browser-use、Stagehand 或 Playwright + AI 视觉理解，为电商卖家、投研机构、品牌方提供定制化网页数据抓取和监控服务。一次开发可复用，边际成本极低。",
        "data_sources": [
            "browser-use GitHub 25k+ stars（2026-05），月下载量 180 万+，社区 800+ 贡献者",
            "Stagehand（browserbase）获 a16z 领投 $15M，专注 AI 驱动的浏览器自动化",
            "淘宝/拼多多/京东商家日均需监控竞品价格、库存、评论，现有工具（八爪鱼/后羿）年费 ¥3,000-12,000",
            "即刻 '爬虫' 话题日均新增 40+ 条，大量用户表示 '不会写代码但需要数据'",
        ],
        "action_steps": [
            "安装 browser-use：pip install browser-use，注册 OpenAI/Anthropic API key，运行官方 demo 熟悉 'AI 控制浏览器' 模式",
            "选择 1 个高价值场景（推荐：Temu/Amazon 竞品价格监控），写出抓取目标字段：标题、价格、评分、库存、最近 10 条评论",
            "用 browser-use 编写抓取 agent：给定 URL 列表 -> AI 自动识别页面结构 -> 提取结构化数据 -> 保存 CSV/JSON，处理反爬（轮换 UA、延时、代理池）",
            "搭建交付界面：Streamlit 单页应用，用户粘贴竞品链接 -> 选择监控频率 -> 开始抓取 -> 下载结果。部署到 Render/Railway 免费档",
            "定价策略：单次抓取 ¥99-499（按页数），监控服务 ¥299-999/月（每日/每周自动更新），企业定制 ¥2,999+",
        ],
...
2. 包含明确的任务描述（agent 知道要找什么）
3. 处理分页和动态加载（滚动/点击"加载更多"）
...
先输出抓取策略说明，再输出完整代码。""",
    ...
    },
    {
        ...
        "margin\": \">75%",
        "summary\": \"用 Flux、Midjourney v7 和 AI 写作工具批量生成高颜值图文内容，覆盖家居、穿搭、职场、育儿等垂直领域，在小红书和公众号做矩阵发布。通过品牌广告、知识付费和私域转化变现。",
        "data_sources": [
        ...
        "action_steps": [
            ...
            "搭建内容工厂：ChatGPT/Claude 写标题和文案（批量生成 50 条）-> Flux/Midjourney 生成封面和配图 -> 剪映做 3-5 秒动态效果 -> 排期发布",
            "建立爆款素材库：用后羿/新榜抓取近 30 天千赞以上图文，分析标题结构（数字法/反常识/痛点型）、封面配色（高饱和度/对比色）、正文节奏（短句+emoji+分段）",
...
请根据以下选题，生成完整的小红书图文内容：
选题：{{topic}}
目标受众：{{target_audience}}

要求：
1. 标题使用数字法或反常识法，含 1-2 个 emoji，不超过 20 字
2. 封面图提示词：给出 Midjourney/Flux 可用的英文 prompt，要求视觉冲击力、高饱和度、主体清晰
3. 正文分 5-7 段，每段不超过 3 行，用 emoji 点缀，语气亲切像闺蜜聊天
4. 结尾带互动钩子（提问或投票）
5. 标签 5-8 个，精准且含 1-2 个大流量标签
...
【互动钩子】""",
    ...
    },
    {
        ...
        "margin\": \">90%",
        "summary\": \"用 AI 批量抓取 Amazon、Temu、速卖通、Shopee 的商品评论，通过情感分析和痛点提取，帮助跨境卖家发现\"高销量低评分\"的改进机会，或找到尚未被满足的细分需求。卖报告和 SaaS 订阅。",
        "data_sources": [
            "Amazon 全球 3P 卖家超 200 万，90% 以上表示\"选品是最大痛点\"，现有工具（Jungle Scout/Helium 10）年费 $600-2,400",
        ...
        "action_steps": [
            "确定目标平台（推荐：Temu 美国站），用 Playwright/browser-use 抓取 TOP 100 商品的评论数据（评分、文字、图片数量、是否带\"Verified Purchase\"）",
            "设计 AI 分析流水线：原始评论 -> 去重清洗 -> 按星级分类 -> LLM 提取痛点关键词（'容易坏'/'尺寸不准'/'说明书看不懂'）-> 统计频率 -> 生成改进建议",
            "输出\"选品雷达报告\"模板：市场概况（总评论数/平均星级/价格带）-> 差评 TOP10 痛点 -> 改进产品概念（现有产品 + 解决痛点 = 新品机会）-> 预估利润空间",
...
平台：{{platform}}
类目：{{category}}
TOP 商品列表（含 ASIN/ID、标题、价格、评分、评论数）：
{{product_list}}
...
3. 评估改进后的新品在市场上的竞争力和预估利润率
4. 输出一份可直接发给卖家的 PDF 风格 Markdown 报告
...
## 行动建议与风险提示""",
    ...
    },
    {
        ...
        "margin\": \">80%",
        "summary\": \"用 ElevenLabs、NotebookLM 或国内讯飞/魔音工坊，将热门文章、公版书籍、行业报告批量转化为高质量有声书和播客节目，分发到小宇宙、喜马拉雅、Spotify、YouTube。通过广告分成、付费订阅和知识付费变现。",
        "data_sources": [
            "喜马拉雅 2025 年报：有声书用户 4.2 亿，付费率 18%，\"AI 合成语音\"内容增速 340%",
            "小宇宙播客 2026 年活跃主播 50 万+，\"商业/科技\"品类广告 CPM ¥80-200",
            "ElevenLabs 2026 年 5 月数据：支持 32 种语言、1,000+ 声音克隆，API 成本 $0.08/千字符（约 10 分钟音频 $0.5）",
            "YouTube Podcasts 月活 10 亿+，创作者可通过 AdSense 获得 $2-8/千次播放（英语内容更高）",
        ],
        "action_steps": [
            "选择内容赛道（推荐：商业思维/AI 趋势/心理学），收集 50 篇高阅读量公版文章/报告（确保无版权风险）",
            "用 ElevenLabs 训练固定主播声音（或使用 API 预设声音），设定参数：语速 1.1x、语气亲和、停顿自然。生成测试音频，确保 10 分钟以上无疲劳感",
            "搭建流水线：原文 -> Claude 改写为口语化播客脚本（加入过渡句、口语化表达、互动提问）-> ElevenLabs API 生成音频 -> ffmpeg 添加片头片尾/背景音乐 -> 生成封面图 -> 批量上传",
            "分发矩阵：小宇宙（中文主阵地）+ 喜马拉雅（长尾流量）+ YouTube Podcasts（英语内容/ AdSense）+ Spotify（全球化）。每个平台每天 1 更，内容可复用",
...
2. 每 300 字插入一个过渡句或口语化衔接（"说到这里"、"你可能想问"、"举个例子"）
3. 关键数据重复一遍（音频无法回看）
...
[片尾音乐 + 订阅引导]""",
    ...
    },
    {
        ...
        "margin\": \">85%",
        "summary\": \"针对 2025-2026 年持续的人才市场波动，为中高端求职者提供 AI 驱动的简历重构、面试模拟和谈薪辅导。用 Claude 4 做简历诊断和模拟面试，用语音工具做实战演练，客单价高、复购低但转介绍高。",
        "data_sources": [
            ...
            "小红书 \"简历修改\"话题浏览量 12 亿+，\"面试技巧\"话题 8 亿+，求职内容需求极其旺盛",
        ],
        "action_steps": [
            ...
            "搭建交付流程：用户填表（目标岗位/当前痛点）-> AI 生成初版简历 -> 人工精修（突出数据和成果）-> 交付 + 30 分钟视频讲解修改逻辑 -> 模拟面试（用语音 Agent 扮演面试官）",
            "获客渠道：小红书发 \"简历前后对比\" 图文（敏感信息打码）+ 知乎写 \"HR 视角：什么样的简历 10 秒过初筛\" + 即刻发 \"AI 模拟面试实录\" 片段 + 脉脉职场圈子",
...
SCHEDULE = [
    {
        "day\": \"周一",
        "theme\": \"新机会首发",
        "title\": \"首发独占：AI 语音电话 Agent SaaS — 7 天搭建、月收 ¥8,000+ 的完整路径",
        "opportunity\": \"opp-v8-001",
        "exclusive\": \"48 小时内专业版独占 | 附赠 Vapi 工作流配置 + 房产中介话术 JSON + 飞书通知模板",
    },
    {
        "day\": \"周二",
        "theme\": \"技术掘金",
        "title\": \"browser-use 实战：如何用 AI 浏览器自动化接下第一个 ¥5,000 的数据抓取项目",
        "opportunity\": \"opp-v8-002",
        "exclusive\": \"完整 browser-use agent 源码 + 反爬策略清单 + 客户报价模板",
    },
    {
        "day\": \"周三",
        "theme\": \"社媒变现",
        "title\": \"小红书矩阵：1 人运营 20 个号，从 0 到月收 ¥10,000+ 的 SOP",
        "opportunity\": \"opp-v8-003",
        "exclusive\": \"50 个可直接使用的标题模板 + Flux 封面 Prompt 库 + 排期表 Excel",
    },
    {
        "day\": \"周四",
        "theme\": \"跨境选品",
        "title\": \"AI 评论分析选品雷达：从 10 万条差评里挖出下一个爆款",
        "opportunity\": \"opp-v8-004",
        "exclusive\": \"完整选品报告模板 + Temu 抓取代码 + 痛点分析 Prompt",
    },
    {
        "day\": \"周五",
        "theme\": \"音频资产",
        "title\": \"AI 播客内容工厂：批量生产有声书，睡后收入的搭建指南",
        "opportunity\": \"opp-v8-005",
        "exclusive\": \"ElevenLabs 批量生成脚本 + 多平台分发 checklist + 变现组合策略",
    },
    {
        "day\": \"周六",
        "theme\": \"深度专题",
        "title\": \"深度专题：裁员潮下的 AI 求职陪跑服务，客单价 ¥1,999 的交付全案",
        "opportunity\": \"opp-v8-006",
        "exclusive\": \"简历重构 Prompt 库 + 模拟面试语音 Agent 配置 + 客户签约合同模板",
    },
    {
        "day\": \"周日",
        "theme\": \"复盘+预告",
        ...
        "opportunity": None,
        "exclusive\": \"会员答疑精华整理 + 下周新机会内幕预告（AI 硬件代理 / 本地生活 Agent / 跨境独立站）",
    },
]
...
def generate_day_report(day_info: dict) -> str:
    ...
    if opp:
        ...
        content = f"""# AI赚钱机会雷达 - {day_info['day']}日报 v8
...
|------|-----------|-------------|---------|
| opp-v8-001 AI 语音电话 Agent | 23% | 已有 2 位会员完成 Vapi demo | 新增教培话术模板 |
| opp-v8-002 browser-use 爬虫 | 18% | 1 位会员接到首单 ¥800 | 新增亚马逊评论分析模块 |
| opp-v8-003 小红书矩阵 | 31% | 3 位会员账号破 500 粉 | 新增 Flux 封面 Prompt 库 |
| opp-v8-004 跨境选品雷达 | 12% | 1 位会员发现潜力品类 | 新增 Shopee 数据抓取 |
| opp-v8-005 音频内容工厂 | 15% | 2 位会员上架首批播客 | 新增小宇宙运营 SOP |
| opp-v8-006 求职陪跑 | 28% | 5 位会员完成简历重构 | 新增模拟面试语音 Agent |
...
**Q: 没有编程基础，能做什么？**
A: 优先执行 opp-v8-003（小红书矩阵）和 opp-v8-006（求职陪跑），两者均无需写代码，只需使用现成工具。

**Q: 启动资金最少需要多少？**
A: ¥0-500。大多数机会可用免费工具起步（Vapi 免费额度、browser-use 开源、小红书注册免费、ElevenLabs 免费试用）。
...
def generate_delivery_checklist() -> str:
    now = datetime.now().strftime('%Y-%m-%d %H:%M')
    files = [
        ("reports/sample_pack/free_preview_v8.md", "免费试看版 Markdown"),
        ("reports/sample_pack/premium_catalog_v8.md", "专业版订阅目录 Markdown"),
        ("reports/sample_pack/data_v8.json", "结构化数据 JSON"),
        ("reports/sample_pack/week1_samples/monday_v8.md", "周一日报样例"),
        ("reports/sample_pack/week1_samples/tuesday_v8.md", "周二日报样例"),
        ("reports/sample_pack/week1_samples/wednesday_v8.md", "周三日报样例"),
        ("reports/sample_pack/week1_samples/thursday_v8.md", "周四日报样例"),
        ("reports/sample_pack/week1_samples/friday_v8.md", "周五日报样例"),
        ("reports/sample_pack/week1_samples/saturday_v8.md", "周六日报样例"),
        ("reports/sample_pack/week1_samples/sunday_v8.md", "周日复盘样例"),
        ("docs/delivery_checklist.md", "交付清单（本文件）"),
    ]
    file_table = "\n".join([f"| {i+1} | `{p}` | {d} |" for i, (p, d) in enumerate(files)])

    return f"""# 交付清单 - knowledge-subscription 首批可售卖内容样例包 v8

**任务ID**: {TASK_ID}  
**版本**: {VERSION}  
**生成时间**: {now}  
**生成器**: app/sample_pack_generator_d03eb155.py
...
python app/sample_pack_generator_d03eb155.py
```

### 3.2 验证 JSON 完整性

```bash
python -c "import json; json.load(open('reports/sample_pack/data_v8.json')); print('JSON OK')"
```

### 3.3 运行内容质量测试

```bash
pytest tests/test_sample_pack_d03eb155.py -v --tb=short
```
...
```bash
# 免费试看版（用于引流）
cat reports/sample_pack/free_preview_v8.md

# 专业版目录（用于销售页）
cat reports/sample_pack/premium_catalog_v8.md

# 首周日报样例
cat reports/sample_pack/week1_samples/monday_v8.md
```
...
| 文件 | 使用场景 | 变现动作 |
|------|----------|----------|
| free_preview_v8.md | 社交媒体/社群引流 | 发布小红书/即刻/知乎，文末放订阅入口 |
| premium_catalog_v8.md | 销售页/落地页素材 | 提取定价表和 FAQ 到 site/index.html |
| week1_samples/*.md | 邮件/Substack/小报童首发内容 | 直接作为首周付费内容发布 |
| data_v8.json | 后续自动化生成器的输入数据源 | 接入每日报告生成流水线 |
...
1. **本周内**：将 free_preview_v8.md 转化为 3 篇引流帖（小红书/即刻/知乎），文末放订阅意向收集表（腾讯问卷/金数据）
2. **本周内**：将 premium_catalog_v8.md 的定价表和 FAQ 更新到 site/index.html
...
def main():
    ...
    write_file(OUTPUT_DIR / "free_preview_v8.md", generate_free_preview())

    print("\n[2/6] Generating premium catalog...")
    write_file(OUTPUT_DIR / "premium_catalog_v8.md", generate_premium_catalog())
    ...
    write_file(OUTPUT_DIR / "data_v8.json", json.dumps(data, ensure_ascii=False, indent=2))
